...
namespace QuantConnect 
{
    ...
    { 
        ...
        public override void Initialize() 
        {
            //AddSecurity(MarketType.Equity, ""AAPL"", Resolution.Tick);
            //AddSecurity(MarketType.Equity, ""AAPL"", Resolution.Second);
            AddSecurity(MarketType.Equity, ""AAPL"", Resolution.Minute);
		...
		//  -> You can only have one resolution per symbol, i.e. Apple Tick or Second, but not both.
        /// </summary>
        /// <param name=""stockData"">Dictionary of TradeBar Class Data Packets Indexed by the Stock Symbol</param>
        public override void OnTradeBar(Dictionary<string, TradeBar> stockData) 
        {
            //Get the Apple TradeBar Object: This contains; open, high, low, close, volume.
            try 
            {
                TradeBar applTradeBar = stockData[""AAPL""];
                decimal holdingAveragePrice = Portfolio[""AAPL""].AveragePrice;
                
                if (Portfolio.HoldStock == false) 
                {
                    Order(""AAPL"", 10);
                    Debug(""Bought 10 Apple at "" + applTradeBar.close);
                    return;
                }
                else if (!messageSent) 
                {
                    //Order is processed on leaving this function/loop.
                    
                    Debug(""Holding "" + Portfolio[""AAPL""].Quantity + 
                          "" stock at: "" + holdingAveragePrice.ToString(""C"") + 
                          "" for total value of "" + Portfolio[""AAPL""].HoldingValue);
		...
		//  -> If a quote tick"" has the properties: decimal dBidPrice, dAskPrice, int iAskQuantity, iBidQuantity, string sBidMarket, sAskMarket 
        /// </summary>
        /// <param name=""tick"">Tick class containing a single trade tick from your a symbol symbol</param>
